...
const dismissed = ref(localStorage.getItem('cendekia-pwa-dismissed') === '1')
...
function dismiss() { dismissed.value = true; localStorage.setItem('cendekia-pwa-dismissed', '1') }
</script>
    ...
    <div v-if="show" class="fixed z-50 left-3 right-3 bottom-3 sm:left-auto sm:right-6 sm:bottom-6 sm:max-w-sm safe-bottom no-print">
      <div class="card p-3.5 flex items-center gap-3 shadow-xl">
        ...
        <div class="min-w-0 flex-1">
          <div class="text-[0.86rem] font-semibold leading-tight">Instal Cendekia LMS</div>
    ...
    <div v-if="iosSheet" class="fixed inset-0 z-[70] flex items-end sm:items-center justify-center p-4 no-print" @click.self="iosSheet = false">
      ...
      <div class="relative card p-5 w-full sm:max-w-sm safe-bottom">
        ...
        <ol class="flex flex-col gap-3 text-sm text-muted">
          ...
          <li class="flex gap-3"><span class="grid h-6 w-6 place-items-center rounded-full text-[0.72rem] font-bold shrink-0" style="background: var(--brand-50); color: var(--brand-600)">3</span> Ketuk <b class="text-ink">Tambah</b> — ikon Cendekia LMS muncul di layar utama.</li>
        </ol>
